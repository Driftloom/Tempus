...
from app.auth.authorization import check_resource_ownership, verify_user_owns_resource
...
@pytest.mark.asyncio
async def test_resource_ownership_check():
    """Test resource ownership verification function."""
    # User owns resource
    assert check_resource_ownership("user123", "user123") is True
    
    # User does not own resource
    assert check_resource_ownership("user123", "user456") is False


@pytest.mark.asyncio
async def test_verify_user_owns_resource():
    """Test database-based resource ownership verification."""
    mock_db = AsyncMock(spec=AsyncSession)
    
    # Should return True (placeholder implementation)
    result = await verify_user_owns_resource(mock_db, "user123", "task", "task1")
    assert result is True


@pytest.mark.asyncio
async def test_task_update_without_ownership():
    """Test that users cannot update tasks they don't own."""
    async with AsyncClient(app=app, base_url="http://test") as client:
        # Mock authentication for user123
        with patch('app.auth.dependencies.get_current_user', return_value="user123"):
            response = await client.patch(
                "/tasks/task456",
                json={"title": "Updated title"},
                headers={"Authorization": "Bearer fake_token"}
            )
            # Should fail ownership check (403) or not found (404)
