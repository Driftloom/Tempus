...
class CacheService:
    ...
    async def invalidate_memory_cache(self, user_id: str, memory_id: str | None = None) -> int:
        """Invalidate memory cache."""
        if memory_id:
            return await self.delete(f"memory:{user_id}:{memory_id}")
        return await self.invalidate_pattern(f"memory:{user_id}")

    async def invalidate_task_cache(self, user_id: str, task_id: str | None = None) -> int:
        """Invalidate task cache."""
        if task_id:
            return await self.delete(f"task:{user_id}:{task_id}")
        return await self.invalidate_pattern(f"task:{user_id}")

    async def invalidate_connector_cache(self, user_id: str, connector_id: str | None = None) -> int:
